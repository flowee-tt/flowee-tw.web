...
class AppState {
  constructor() {
    ...
    this.usersProfiles = JSON.parse(localStorage.getItem('web_lop_users_profiles')) || {};
    ...
    this.hasCloudBeenFetched = false;
  }
  ...
  save(broadcast = true, forcePush = false) {
    if (this.currentUser && this.currentUser.realName) {
      this.usersProfiles[this.currentUser.realName] = {
    ...
    localStorage.setItem('web_lop_users_profiles', JSON.stringify(this.usersProfiles));
  ...
  async pushToCloud(force = false) {
    if (!this.hasCloudBeenFetched && !force) {
      console.log("⏳ Skipping cloud push until initial cloud fetch merges.");
      return;
    }
      ...
      if (res && res.ok) {
        ...
        if (cloudData) {
          ...
          if (cloudData.usersProfiles) {
            this.usersProfiles = { ...this.usersProfiles, ...cloudData.usersProfiles };
            if (this.currentUser && this.currentUser.realName && this.usersProfiles[this.currentUser.realName]) {
              const prof = this.usersProfiles[this.currentUser.realName];
              if (prof.nickname !== undefined) this.currentUser.nickname = prof.nickname;
              if (prof.avatar !== undefined) this.currentUser.avatar = prof.avatar;
            }
            hasChanges = true;
          }
...
function setupRealtimeSyncEngine() {
  ...
  // Instant Global Cloud Polling every 2 seconds (when tab is active)
  setInterval(() => {
    if (document.visibilityState === 'visible') {
      state.fetchFromCloud();
    }
    ...
    userBtn.innerHTML = `
      ...
      <div class="hidden sm:block text-left">
        <div class="text-xs font-bold text-slate-100 flex items-center gap-1">
          ${displayName}
          ${state.currentUser.isSuperAdmin ? '<span class="text-[10px] bg-amber-500 text-slate-950 px-1.5 rounded font-black">SUPER ADMIN</span>' : (state.currentUser.isAdmin ? '<span class="text-[10px] bg-amber-500/20 text-amber-300 border border-amber-500/40 px-1.5 rounded font-extrabold">ADMIN LỚP</span>' : '')}
        </div>
...
window.submitUploadForm = function() {
  ...
  closeUploadModal();
  renderApp();
  showToast("🎉 Đã đăng tải bài kỷ niệm & video clip! Đang đồng bộ tới tất cả thiết bị...");
};
...
// ----------------------------------------------------
// 7. AUTHENTICATION & GATEWAY HANDLERS (DUAL ACCESS SYSTEM)
// ----------------------------------------------------
window.openSuperAdminModalDirect = function() {
  openSuperAdminModal();
};
...
function renderSuperAdminModalContent() {
  const container = document.getElementById('super-admin-body');
  const totalClasses = state.classesIndex.length;

  container.innerHTML = `
    <div class="space-y-5 text-xs">
      
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div class="bg-slate-900 p-3.5 rounded-2xl border border-amber-500/30 text-center">
          <div class="text-[10px] text-slate-400 font-bold uppercase">Tổng Lớp Đang Quản Lý</div>
          <div class="text-xl font-black text-amber-400 mt-1">${totalClasses} Lớp</div>
        </div>
        <div class="bg-slate-900 p-3.5 rounded-2xl border border-indigo-500/30 text-center">
          <div class="text-[10px] text-slate-400 font-bold uppercase">Đang Xem Lớp Trực Tiếp</div>
          <div class="text-xl font-black text-indigo-400 mt-1">${state.systemClassName}</div>
        </div>
        <div class="bg-slate-900 p-3.5 rounded-2xl border border-emerald-500/30 text-center">
          <div class="text-[10px] text-slate-400 font-bold uppercase">Trạng Thái Hệ Thống</div>
          <div class="text-xl font-black text-emerald-400 mt-1">Hoạt Động 🟢</div>
        </div>
      </div>
      ...
      <div class="bg-slate-900 p-4 rounded-2xl border border-amber-500/40 space-y-3">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label class="block text-slate-300 font-semibold mb-1">Mã Lớp Hệ Thống (VD: 10A6, 12A1):</label>
            <input type="text" id="super-new-sys-name" placeholder="VD: 10A6" class="w-full bg-slate-950 border border-slate-700 rounded-xl p-2.5 text-slate-100 focus:outline-none focus:border-amber-500">
          </div>
          <div>
            <label class="block text-slate-300 font-semibold mb-1">Tên Hiển Thị Web Lớp:</label>
            <input type="text" id="super-new-web-name" placeholder="VD: Lớp 10A6 Kỷ Niệm" class="w-full bg-slate-950 border border-slate-700 rounded-xl p-2.5 text-slate-100 focus:outline-none focus:border-amber-500">
          </div>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div>
            <label class="block text-slate-300 font-semibold mb-1">Họ Tên Admin Lớp:</label>
            <input type="text" id="super-new-admin-name" placeholder="VD: Nguyễn Văn Nam" class="w-full bg-slate-950 border border-slate-700 rounded-xl p-2.5 text-slate-100 focus:outline-none focus:border-amber-500">
          </div>
          <div>
            <label class="block text-slate-300 font-semibold mb-1">Cấp Mật Khẩu Admin Lớp:</label>
            <input type="text" id="super-new-admin-pass" value="AdminA6-99" class="w-full bg-slate-950 border border-slate-700 rounded-xl p-2.5 text-amber-300 font-bold focus:outline-none focus:border-amber-500">
          </div>
          <div>
            <label class="block text-slate-300 font-semibold mb-1">Giới Hạn Dung Lượng (GB):</label>
            <input type="number" id="super-new-gb-limit" value="5.0" step="0.5" class="w-full bg-slate-950 border border-slate-700 rounded-xl p-2.5 text-slate-100 focus:outline-none focus:border-amber-500">
          </div>
        </div>
      ...
      <div class="bg-slate-900 p-4 rounded-2xl border border-slate-800 space-y-3">
        <h4 class="font-bold text-slate-200 text-sm">🏢 Quản Lý Các Lớp, Mật Khẩu Admin & Dung Lượng (${state.classesIndex.length})</h4>

        <div class="space-y-3 max-h-64 overflow-y-auto">
          ${state.classesIndex.map(c => {
            const classGbQuota = c.gbQuota || 5.0;
            const usageInfo = state.calculateClassStorage(c.systemName);
            const currentGb = parseFloat(usageInfo.gb);
            const pct = Math.min(100, Math.round((currentGb / classGbQuota) * 100));
            const isExceeded = currentGb > classGbQuota;
            const isCurrentActive = (state.systemClassName === c.systemName);

            return `
              <div class="p-3 bg-slate-950 rounded-xl border ${isCurrentActive ? 'border-amber-500 ring-1 ring-amber-500/50' : (isExceeded ? 'border-rose-500/80 bg-rose-950/10' : 'border-slate-800')} space-y-2">
                <div class="flex items-center justify-between">
                  <div>
                    <div class="font-bold text-amber-300 text-sm flex items-center gap-2">
                      <span>${c.webName}</span>
                      <span class="text-slate-400 text-xs">(${c.systemName})</span>
                      ${isCurrentActive ? '<span class="text-[9px] bg-amber-500 text-slate-950 font-extrabold px-1.5 py-0.5 rounded">ĐANG XEM</span>' : ''}
                    </div>
                    
                    <div class="flex items-center gap-3 text-[10px] text-slate-400 mt-1">
                      <span>Admin Lớp: <strong class="text-slate-200">${c.adminName}</strong></span>
                      <span>Mật khẩu Admin Lớp: <strong class="font-mono text-amber-300 bg-slate-900 px-1.5 py-0.5 rounded border border-amber-500/30">${c.adminPass || 'AdminA6-99'}</strong></span>
                      <button onclick="superAdminUpdateAdminPass('${c.id}')" class="text-indigo-400 hover:underline font-bold">Đổi MK Admin</button>
                    </div>
                  </div>

                  <div class="flex items-center gap-2">
                    <button onclick="superAdminEnterClass('${c.id}')" class="px-3 py-1.5 ${isCurrentActive ? 'bg-amber-500 text-slate-950' : 'bg-indigo-600 hover:bg-indigo-500 text-white'} font-bold text-xs rounded-lg">
                      ${isCurrentActive ? 'Đang Ở Lớp Này' : 'Vào Xem Lớp'}
                    </button>
                    <button onclick="superAdminDeleteClass('${c.id}')" class="px-2.5 py-1.5 bg-rose-500/20 text-rose-300 border border-rose-500/40 hover:bg-rose-500/40 font-bold text-xs rounded-lg">
                      🗑️ Xóa Lớp
                    </button>
                  </div>
                </div>
                ...
                <div class="space-y-1">
                  <div class="flex justify-between text-[10px] font-bold">
                    <span class="text-slate-400">Dung lượng sử dụng:</span>
                    <span class="${isExceeded ? 'text-rose-400 font-black' : 'text-emerald-400'}">${currentGb} GB / ${classGbQuota} GB (${pct}%)</span>
                  </div>
                  <div class="w-full bg-slate-800 rounded-full h-1.5 overflow-hidden">
                    <div class="${isExceeded ? 'bg-rose-500' : 'bg-emerald-500'} h-1.5 rounded-full" style="width: ${pct}%"></div>
                  </div>
                </div>
              </div>
            `;
...
window.superAdminUpdateAdminPass = function(classId) {
  const c = state.classesIndex.find(x => x.id === classId);
  if (!c) return;
  ...
  if (newPass && newPass.trim()) {
    ...
    if (c.systemName === state.systemClassName) {
      state.classAdminPassword = newPass.trim();
    }
    state.save(true);
    renderSuperAdminModalContent();
...
window.superAdminCreateClass = function() {
  const sysName = document.getElementById('super-new-sys-name').value.trim();
  const webName = document.getElementById('super-new-web-name').value.trim();
  const adminName = document.getElementById('super-new-admin-name').value.trim();
  const adminPass = document.getElementById('super-new-admin-pass').value.trim();
  const gbLimit = parseFloat(document.getElementById('super-new-gb-limit').value) || 5.0;
...
window.superAdminEnterClass = async function(classId) {
  const targetClass = state.classesIndex.find(c => c.id === classId);
  if (!targetClass) return;

  state.switchClass(targetClass.systemName);
  await hydratePostMediaUrls();

  closeSuperAdminModal();
  renderApp();
  showToast(`🌐 Đã chuyển sang không gian web lớp: ${targetClass.webName} (${targetClass.systemName})`);
};
...
window.handleWhitelistLogin = function() {
  ...
  // 1. SUPER ADMIN PIN BYPASS DIRECT ON MAIN WEB
  if (passInput === SUPER_ADMIN_PIN || passInput === 'super999') {
    state.currentUser = {
      realName: "Super Admin",
      nickname: "Ban Điều Hành",
      avatar: "https://api.dicebear.com/7.x/bottts/svg?seed=SuperAdmin",
      role: "🏢 SUPER ADMIN HỆ THỐNG",
      isLoggedIn: true,
      isAdmin: true,
      isSuperAdmin: true
    };
    ...
    renderApp();
    openSuperAdminModal();
    showToast("🏢 Chào mừng Super Admin vào Trung Tâm Điều Khiển Commercial!");
    return;
  }
...
function setupEventListeners() {
  ...
  document.getElementById('global-search').addEventListener('input', (e) => {
    ...
    if (state.searchQuery.trim() === 'super999') {
      openSuperAdminModal();
      e.target.value = '';
      return;
    }
